...
export default memo(function Timer({
  ...
  return <>
      ...
      <div className="flex gap-2 h-12 mt-6">
        <button className="w-20 backdrop-blur-sm z-20 rounded-md border hover:bg-zinc-800 hover:text-white border-zinc-500 text-slate-400 transition-all duration-150" onClick={toggleTimer}>
          {!timerStart ? "Start" : "Pause"}
        </button>

        <button className="w-20 backdrop-blur-sm rounded-md z-20 border border-red-400 hover:bg-red-400/20 text-red-400 transition-all duration-150" onClick={triggerResetDialog}>
          Reset
        </button>
      </div>
      {modal ? <div className="border border-zinc-400 bg-zinc-900/70 shadow-md duration-150 backdrop-blur-lg transition-all rounded-md z-30 p-4 absolute top-16 right-4 ">
