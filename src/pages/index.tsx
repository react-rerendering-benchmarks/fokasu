...
const App = memo(function App() {
  ...
  return <div className="bg-zinc-900 h-screen mx-auto flex flex-col items-center backdrop-blur-md">
      <div className="flex relative w-full h-20">
        <a className="text-zinc-400 absolute top-5 left-5 hover:bg-zinc-400/20 rounded-md hover:ring transition-all duration-150 hover:ring-zinc-400 z-30 animate-pulse" href="https://github.com/terrnit" target="_blank">
          <AiOutlineGithub size={35} />
        </a>
        <button className="text-zinc-400 absolute top-5 right-5 hover:bg-zinc-400/20 rounded-md hover:ring transition-all duration-150 hover:ring-zinc-400 z-30" onClick={() => setModal(prev => !prev)}>
          <CgMenuRight size={35} />
        </button>
      </div>
